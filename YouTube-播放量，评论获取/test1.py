...
def get_html(url):
    ...
    if html.status_code == 200:
        ...
        try:
            ...
            views = 0
        #获取点赞人数
        accept = re.compile('"accessibility":{"accessibilityData":{"label":".*?"}},"simpleText":"(\d+)"},"defaultServiceEndpoint"')
        accepts = accept.findall(content)
        try:
            accepts = accepts[0].replace(',','')

        except:
            accepts = 0
        #获取评论人数
        comment = re.compile('"commentCount":{"simpleText":"(\d+)"},"teaserAvatar"')
        ...
        try:
            ...
            comments = 0
        #获取粉丝数量
        fan = re.compile('{"accessibility":{"accessibilityData":{"label":".*?订阅者"}},"simpleText":"(.*?)"}')
        ...
        # 发布者
        name = re.compile('"subscribeAccessibility":\{"accessibilityData":\{"label":"订阅“(.*?)”。"}},"unsubscribeAccessibility":')
        ...
        return url,views,accepts,comments,fans,titles,timedates,names
    else:
        print(html.status_code)


def data_screen():
    # df = pd.read_csv('./input/{}'.format(input_filename), encoding="UTF-16",sep='\t').iloc[0:1]
    df = pd.read_excel('./input/{}'.format(input_filename),sheet_name='lucky draw')
    data = df
    ...
    #     futuers = [e.submit(get_html, item,timedate) for item,timedate in zip(data['URL'],data['Alternate Date Format'])]
    #     # 用叠带把这个函数的内容一个个打印出来
    #     for futuer in tqdm(concurrent.futures.as_completed(futuers)):
    #         timedate,url,views,accepts,comments,fans = futuer.result()
    #         list_date.append(timedate)
    #         list_url.append(url)
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as e:
        # 调用这个函数，用一个列表的形式把这个函数给保存下来
        futuers = [e.submit(get_html, item) for item in data['Youtube链接']]
        # 用叠带把这个函数的内容一个个打印出来
        for futuer in tqdm(concurrent.futures.as_completed(futuers)):
            url,views,accepts,comments,fans,titles,timedates,names = futuer.result()
...
if __name__ == '__main__':
    input_filename = 'CODM-LD&BP舆情监控需求.0708.xlsx'
    # 数据筛选
    data_screen()
